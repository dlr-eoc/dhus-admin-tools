...
function usage {
  ...
  >&2 echo "  --limit the amount of products to be retrieved (default=50)"
  ...
  >&2 echo ""
  >&2 echo "Example call to number each line and unwrap message lines:"
  >&2 echo -n "./dhus-deleted-inventory.sh -w=.apihubrc -l=2000 -b=100 | awk '"
...
# defaults
condition=''
select="Id,Name,Size,IngestionDate,ContentDate,DeletionDate,DeletionCause,Checksum"
...
omitHeader=0
skip=""
odataQuery="\$top=$batchsize${select:+&\$select=$select}&\$format=text/csv&\$orderby=DeletionDate asc${condition:+&\$filter=$condition}"
pos=0
while [ $pos -le $limit ]
do
